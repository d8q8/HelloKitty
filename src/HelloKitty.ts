...
class HelloKitty extends egret.DisplayObjectContainer {
    ...
    private init() {
        //心形状
        var line:lcp.LCircle = new lcp.LCircle({x: 100, y: 200, radius: 50, fillcolor: 0xff0000, thickness: 10, linecolor: 0x00ff00});
        this.addChild(line);
        // 画圆角
        var sp = new lcp.LRoundRect({name: "sp", x: 100, y: 500, width: 200, height: 100, ellipseWidth: 100, ellipseHeight: 50, fillcolor: 0xff0000, thickness: 10, linecolor: 0x00ff00});
        this.addChild(sp);
        ...
        //框架自带的缓动类
        egret.Tween.get(sp).to({x:200,scaleX:.5,scaleY:.5,rotation:360},1000).call(function(aaa){
            lcp.LTrace.trace("我回调了[缓动类],哟西!",aaa);
        },this,[1,2,3]);



        //var tweenlite1:TweenLite = TweenLite.to(sp, 1, {x: 100, y: 500, scaleX: .5, scaleY: .5, rotation: 360, onComplete: (e)=> {
        //    lcp.LTrace.trace("动画完成回调之后：", sp.x, sp.y, sp.width, sp.height);
        //}});

        //画方
        /*var shp = new lcp.LRoundRect({x:250,y:200,width:200,height:100,radius:20,fillcolor:0x00ff00,thickness:10,linecolor:0xff0000});
         this.addChild(shp);
         ...
         var tweenlite2 = TweenLite.to(shp,1,{x:500,y:500,scaleX:.5,scaleY:.5,rotation:360,onComplete:(e)=>{
